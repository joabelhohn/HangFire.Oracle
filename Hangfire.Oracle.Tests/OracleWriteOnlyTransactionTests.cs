...
{
    public class OracleWriteOnlyTransactionTests : IClassFixture<TestDatabaseFixture>
    {
        private readonly PersistentJobQueueProviderCollection _queueProviders;

        public OracleWriteOnlyTransactionTests()
        {
        ...
        public void Ctor_ThrowsAnException_IfStorageIsNull()
        {
            var exception = Assert.Throws<ArgumentNullException>(
                () => new OracleWriteOnlyTransaction(null));
        ...
        public void ExpireJob_SetsJobExpirationData()
        {
            ...
            UseConnection(sql =>
            {
                ...
                var jobId = param.Get<int>("id").ToString();

                var param2 = new DynamicParameters();
                param2.Add("id", dbType: DbType.Int32, direction: ParameterDirection.Output);
                sql.Execute(arrangeSql, param2);
                var anotherJobId = param2.Get<int>("id").ToString();
        ...
        public void IncrementCounter_WithExistingKey_AddsAnotherRecord()
        {
            UseConnection(sql =>
            {
                ...
                var recordCount = sql.Query<int>("select count(*) from HANGFIRE_COUNTER").Single();
        ...
        public void AddToSet_AddsARecord_WhenKeyIsExists_ButValuesAreDifferent()
        {
            UseConnection(sql =>
            {
                ...
                var recordCount = sql.Query<int>("select count(*) from HANGFIRE_SET").Single();
        ...
        public void InsertToList_AddsAnotherRecord_WhenBothKeyAndValueAreExist()
        {
            UseConnection(sql =>
            {
                ...
                var recordCount = sql.Query<int>("select count(*) from HANGFIRE_List").Single();
        ...
        public void RemoveHash_RemovesAllHashRecords()
        {
            UseConnection(sql =>
            {
                ...
                // Assert
                var count = sql.Query<int>("select count(*) from HANGFIRE_Hash").Single();
        ...
        public void AddRangeToSet_AddsAllItems_ToAGivenSet()
        {
            UseConnection(sql =>
            {
                ...
                var records = sql.Query<string>(@"select Value from HANGFIRE_SET where Key = N'my-set'");
...
insert into HANGFIRE_SET (KEY, VALUE, Score) values (:key, :value, 0.0)";
        ...
        {
            const string arrangeSql = @"
insert into HANGFIRE_HASH (KEY, FIELD, ExpireAt)
values (:key, :field, :expireAt)";
        ...
        {
            const string arrangeSql = @"
insert into HANGFIRE_SET (KEY, VALUE, ExpireAt, Score) values (:key, :value, :expireAt, 0.0)";
...
insert into HANGFIRE_List (KEY, ExpireAt) values (:key, :expireAt)";
        ...
        private static void UseConnection(Action<OracleConnection> action)
        {
        ...
        private void Commit(
            ...
            Action<OracleWriteOnlyTransaction> action)
        {
            var storage = new Mock<OracleStorage>(connection);
            storage.Setup(x => x.QueueProviders).Returns(_queueProviders);

            using (var transaction = new OracleWriteOnlyTransaction(storage.Object))
            {
                action(transaction);
                transaction.Commit();
            }
