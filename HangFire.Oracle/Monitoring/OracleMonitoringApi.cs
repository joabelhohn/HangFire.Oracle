...
namespace Hangfire.Oracle.Monitoring
{
    internal class OracleMonitoringApi : IMonitoringApi
    {
        ...
        public IList<ServerDto> Servers()
        {
            ...
            {
                var servers = 
                    connection.Query<Entities.Server>("select * from HANGFIRE_SERVER").ToList();
...
select sum(x.VALUE) from (
    ...
    select sum(Value) as Value from HANGFIRE_AGGREGATEDCOUNTER where Key = :key
        ...
        private T UseConnection<T>(Func<OracleConnection, T> action)
        {
...
@"select * from (
  ...
  order by j.Id desc
        ...
        private Dictionary<DateTime, long> GetTimelineStats(OracleConnection connection,
...
from HANGFIRE_JOB j
left join HANGFIRE_STATE s on s.Id = j.StateId
